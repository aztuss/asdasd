...
def initialize():
    ...
    if account_info and "accounts" in account_info and account_info["accounts"]:
        balance = account_info["accounts"][0].get("balance", config.STARTING_CAPITAL)
...
def run_cycle():
    ...
    account_info = api.get_account_info()
    if account_info and "accounts" in account_info and account_info["accounts"]:
        balance = account_info["accounts"][0].get("balance", risk_manager.current_capital)
    ...
    for symbol in config.INSTRUMENTS:
        ...
        base_lot = risk_manager.calculate_lot_size(atr_value)
